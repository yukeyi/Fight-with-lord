...
def lordMustPlay(gameinfo):
    must_play = gameinfo['handRemain'][0] == 1 or gameinfo['handRemain'][0] == 2
    return must_play

def farmerMustPlay(gameinfo):
    must_play = gameinfo['handRemain'][0] == 1
    return must_play
...
def playTriple(pos_split):
    ...
    if value == 0 or value > play_value: #no single or single card > triple
        ...
        if value != 0 and (play_value >= 3 and play_value <= 6) :
            if value - play_value <= 5 or (single_len == 1 and value <= 15 and double_len == 0):
                return list(np.array(play_card) + np.array(single_card))
...
def playSingle(gameinfo, pos_split):
    single_len = len(pos_split['single'])
    no_joker = expApp.noJoker(gameinfo)
    farmer_max = expApp.getSingleMax(gameinfo, pos_split)
    ...
    for i in range(triple_len):
        ...
        if value <= 13:
            small_triple_len += 1
    if small_triple_len >= len(single_value):
        return [0] * 15
    single_value = single_value[small_triple_len: ]
    for i in range(single_len):
        value = getPlayMax(pos_split['single'][i])
...
def playSmall(pos_split):
    ...
    if small_single + small_double - triple_len >= 2 and pos_split['remain_len'] >= 8:
...
def checkSingleMax(gameinfo, max_value):
    for i in range(15):
        if (gameinfo['handCard'][0][i] >= 1) and 17 - i > max_value:
            return False
    return True

def checkDoubleMax(gameinfo, max_value):
    for i in range(15):
        if (gameinfo['handCard'][0][i] >= 2)and 17 - i > max_value:
            return False
    return True
...
def playBombTwo(gameinfo, pos_split):
    ...
    must_play = farmerMustPlay(gameinfo)
    ...
    bigger_num = 0
    for i in range(15):
        if (gameinfo['handCard'][0][i] > 0) and 17 - i > max_value:
            max_value = 17 - i
    small_single = 0
    for i in range(single_len):
        value = getPlayMax(pos_split['single'][i])
...
def playSingleTeammate(gameinfo, pos_split):
    single_len = len(pos_split['single'])
    double_len = len(pos_split['double'])
    small_value = 20
    for i in range(single_len):
    ...
    for i in range(straight_len):
        cnt_len = len(pos_split['straight'][i])
        value = getPlayMin(pos_split['straight'][i])
        if cnt_len > 5 and value < small_value:
            small_value = value
    if small_value >= 14:
        small_value = 20
    for i in range(triple_len):
        ...
        if value == small_value:
            ...
            return final_card
    for i in range(straight_len):
        value = getPlayMax(pos_split['straight'][i])
        if value == small_value:
            final_card = [0] * 15
            final_card[17 - value] = 1
            return final_card
...
def playSingleDouble(gameinfo, pos_split):
    if pos_split['small_double'] >= 2 or pos_split['small_single'] >= 2:
        if pos_split['small_double'] >= pos_split['small_single']:
            has_two = gameinfo['handRemain'][0] == 2
            ...
            if pos_split['depth'] == 1:
                return play_card
            has_one = gameinfo['handRemain'][0] == 1
            if has_one:
                return [0] * 15
            return play_card
    return [0] * 15
...
def activePlay(gameinfo, id):
    pos_split = splitApp.get_split(gameinfo['handCard'][id], expApp.REVERSE_SPLIT)
    if expApp.DEBUG == 1:
        printSplit(pos_split)
        print("")
    expApp.addAditionalInfo(gameinfo, pos_split)
    ...
    must_play = farmerMustPlay(gameinfo)
    if id == 1 and gameinfo['handRemain'][2] == 1 and pos_split['other_depth'] != 1:
        play_card = playSingleTeammate(gameinfo, pos_split)
        if sum(play_card) != 0:
            return play_card
    if must_play == False:
    ...
    if triple_len > 0:
        ...
        if sum(play_card) == 0: #if x very bigger
            value = 20
        if value <= 13:  #less than 10, play 3+x
            return play_card
    ...
    if double_len > 0:
        has_two = gameinfo['handRemain'][0] == 2
        if has_two and pos_split['other_depth'] != 1:
            play_card = playSingle(gameinfo, pos_split)
            value = getPlayMin(play_card)
            if sum(play_card) != 0 and value <= 13:
                return play_card
...
def getNegSingleTeammate(triple_value, pos_split):
    single_len = len(pos_split['single'])
    min_value = 20
    for i in range(single_len):
        cnt_value = getPlayMax(pos_split['single'][i])
    ...
    if min_value != 20 and triple_value >= 3 and triple_value <= 6:
        if min_value - triple_value <= 5:
            return min_value
    if min_value != 20 and triple_value >= 7 and triple_value <= 9:
        if min_value <= 11:
            return min_value
    if min_value != 20 and (pos_split['remain_len'] <= 5 or pos_split['depth'] == 1):
        return min_value
    return min_value if min_value != 20 else 0


def getNegDoubleTeammate(triple_value, pos_split):
    double_len = len(pos_split['double'])
    min_value = 20
    for i in range(double_len):
        ...
        if cnt_value < min_value:
            min_value = cnt_value
    if min_value != 20 and triple_value >= 3 and triple_value <= 6:
        if min_value - triple_value <= 5 or pos_split['depth'] == 1:
            return min_value
    if min_value != 20 and triple_value >= 7 and triple_value <= 10:
        if min_value <= 12 or pos_split['depth'] == 1:
            return min_value
    if min_value != 20 and triple_value >= 11 and pos_split['other_depth'] == 1:
        return min_value
    return 0


def negPlaySingleTeammate(gameinfo, pos_split):
    ...
    single_index = 0
    must_play = farmerMustPlay(gameinfo)
    if must_play and other_value <= 13:
        return expApp.negPlaySingleReverse(gameinfo, pos_split)

    for i in range(single_len):
        cnt_value = getPlayMax(pos_split['single'][i])
        if cnt_value == min_value and (cnt_value <= 14 or pos_split['other_depth'] == 1):
            return pos_split['single'][i]
    return [0] * 15


def negPlayDoubleTeammate(gameinfo, pos_split):
    ...
    for i in range(double_len):
        cnt_value = getPlayMax(pos_split['double'][i])
        if cnt_value == min_value and (cnt_value <= 11 or pos_split['other_depth'] == 1):
            return pos_split['double'][i]
    return [0] * 15


def negPlayTripleTeammate(gameinfo, pos_split):
    ...
    for i in range(triple_len):
        ...
        if cnt_value > other_value and min_value > cnt_value:
            min_value = cnt_value
    if min_value >= 10 and pos_split['other_depth'] != 1:
        return [0] * 15
    for i in range(triple_len):
        cnt_value = getPlayMax(pos_split['triple'][i])
        if cnt_value == min_value:
            if is_single == 1: #3 + x
                single_value = getNegSingleTeammate(cnt_value, pos_split)
                if single_value == 0:
                    return [0] * 15
            ...
            if is_single == -1: # 3
                return pos_split['triple'][i]
            if is_single == 0: # 3 + xx
                double_value = getNegDoubleTeammate(cnt_value, pos_split)
                if double_value == 0:
                    return [0] * 15
...
def negPlayStraightTeammate(gameinfo, pos_split):
    ...
    for i in range(straight_len):
        ...
        if cnt_len == other_len and cnt_value > other_value and min_value > cnt_value:
            min_value = cnt_value
    if min_value > 6 and pos_split['other_depth'] != 1:
        return [0] * 15
    for i in range(straight_len):
        cnt_len = sum(pos_split['straight'][i])
        cnt_value = getPlayMin(pos_split['straight'][i])
...
def negPlayStraightDoubleTeammate(gameinfo, pos_split):
    ...
    for i in range(double_straight_len):
        ...
        if cnt_len == other_len and cnt_value > other_value and min_value > cnt_value:
            min_value = cnt_value
    if min_value > 8 and pos_split['other_depth'] != 1:
        return [0] * 15
    for i in range(double_straight_len):
        cnt_len = sum(pos_split['double_straight'][i]) // 2
        cnt_value = getPlayMin(pos_split['double_straight'][i])
...
def negPlayTeammate(gameinfo, id):
    pos_split = splitApp.get_split(gameinfo['handCard'][id], expApp.REVERSE_SPLIT)
    if expApp.DEBUG == 1:
        printSplit(pos_split)
        print("")
    ...
    if card_type == 2: #single
        other_value = getPlayMax(gameinfo['lastCard'])
        if checkSingleMax(gameinfo, other_value) and gameinfo['handRemain'][gameinfo['last']] == 1:
                return [0] * 15
        ...
        has_one = gameinfo['handRemain'][0] == 1
        if has_one:
            if judgeSingle(gameinfo, pos_split) == True: #if lord have only one card and I have only one small single card, then bomb or rocket
                if bomb_len > 0:
                ...
                other_value = getPlayMax(gameinfo['lastCard'])
                for i in range(15):
                    if gameinfo['handCard'][id][i] > 0 and 17 - i > other_value:
                        final_card = [0] * 15
                        final_card[i] = 1
                        return final_card
        play_card = negPlaySingleTeammate(gameinfo, pos_split)
        if sum(play_card) != 0:
            return play_card
    if card_type == 3: #double
        play_card = negPlayDoubleTeammate(gameinfo, pos_split)
        value = getPlayMax(play_card)
        if sum(play_card) != 0:
            return play_card
    if card_type == 4: #triple
        play_card = negPlayTripleTeammate(gameinfo, pos_split)
        #if pos_split['depth'] <= 3:
        #    return ai2App.getNextMove(gameinfo, id)
        if sum(play_card) != 0:
            return play_card
    if card_type == 5: #straight
        play_card = negPlayStraightTeammate(gameinfo, pos_split)
        if sum(play_card) != 0:
            return play_card
    if card_type == 6: #double straight
        play_card = negPlayStraightDoubleTeammate(gameinfo, pos_split)
        if sum(play_card) != 0:
            return play_card
...
def getNegSingle(triple_value, pos_split, must_play):
    ...
    if min_value != 20 and triple_value >= 12 and triple_value <= 13:
        if min_value < triple_value or must_play:
            return min_value
    if min_value != 20 and (pos_split['remain_len'] <= 5 or pos_split['lord_card'] <= 2):
        return min_value
    ...
    if min_value != 20 and triple_value >= 12 and triple_value <= 13:
        if min_value < triple_value or must_play:
            return min_value
    if min_value != 20 and (pos_split['sum'] <= 5 or pos_split['lord_card'] <= 2):
        return min_value
    return min_value if min_value != 20 else 0
...
def negPlaySingle(gameinfo, pos_split):
    ...
    small_single = 0
    no_joker = expApp.noJoker(gameinfo)
    farmer_must_play = farmerMustPlay(gameinfo)
    for i in range(single_len):
        cnt_value = getPlayMax(pos_split['single'][i])
        if cnt_value <= 13:
            small_single += 1
    ...
    for i in range(single_len):
        cnt_value = getPlayMax(pos_split['single'][i])
        if cnt_value == min_value:
            if (min_value == 16 or min_value == 17) and gameinfo['turn'] == 1 and gameinfo['handRemain'][2] == 1:
                return pos_split['single'][i]
            if (min_value == 16 or min_value == 17) and small_single >= 3 and farmer_must_play == False:
                break
            return pos_split['single'][i]
    double_len = len(pos_split['double'])
    for i in range(double_len):
...
def negPlayDouble(gameinfo, pos_split):
    ...
    for i in range(double_len):
        cnt_value = getPlayMax(pos_split['double'][i])
        if cnt_value == min_value and ((cnt_value <= 14 and cnt_value - other_value <= 11) or farmer_must_play):
            return pos_split['double'][i]
        if cnt_value == min_value and cnt_value == 15 and (pos_split['small_single'] + pos_split['small_double'] <= 3):
            return pos_split['double'][i]
    double_straight_len = len(pos_split['double_straight'])
    for i in range(double_straight_len):
    ...
    for i in range(triple_len):
        ...
        if (value == 14 or value == 15) and value > other_value:
            if farmer_must_play or farmer_card < 8:
                final_card = [0] * 15
...
def negPlayTriple(gameinfo, pos_split):
    ...
    for i in range(triple_len):
        ...
        if cnt_value > other_value and min_value > cnt_value:
            min_value = cnt_value
    if min_value == 15 and min_value - other_value >= 2 and gameinfo['handRemain'][0] >= 3:
        return [0] * 15
    if min_value == 14 and min_value - other_value >= 3 and gameinfo['handRemain'][0] >= 3:
        return [0] * 15
    for i in range(triple_len):
        cnt_value = getPlayMax(pos_split['triple'][i])
        if cnt_value == min_value:
...
def checkCard(gameinfo, pos_split):
    single_len = len(pos_split['single'])
    max_value = 0
    bigger_num = 0
    for i in range(15):
    ...
    if bigger_num < pos_split['small_single']:
        return False
    max_value = 0
    bigger_num = 0
    for i in range(15):
...
def checkTripleMax(gameinfo, triple_value):
    for i in range(15):
        if (gameinfo['handCard'][0][i] >= 3) and 17 - i > triple_value:
            return False
        if gameinfo['handCard'][0][i] == 4:
            return False
    return True
...
def negActiveBombRocket(gameinfo, pos_split):
    ...
    if pos_split['depth'] <= (bomb_len + rocket_len) * 2 + 1:  # one bomb + two others(others can be straight or triple)
        ...
        if big_value >= 1:
            return play_card
        if checkCard(gameinfo, pos_split):
            return play_card
    return [0] * 15

def judgeSingle(gameinfo, pos_split):
    largest_value = getPlayMax(gameinfo['handCard'][0])
    single_len = len(pos_split['single'])
    single = 0
    for i in range(single_len):
        value = getPlayMax(pos_split['single'][i])
        if value < largest_value:
            single += 1
    return single - pos_split['small_triple_len'] <= 1


def negPlayLord(gameinfo, id):
    pos_split = splitApp.get_split(gameinfo['handCard'][id], expApp.REVERSE_SPLIT)
    if expApp.DEBUG == 1:
        printSplit(pos_split)
        print("")
            ...
            if judgeSingle(gameinfo, pos_split) == True: #if lord have only one card and I have only one small single card, then bomb or rocket
                ...
                for i in range(15):
                    if gameinfo['handCard'][id][i] > 0 and 17 - i > other_value:
                        ...
                        final_card[i] = 1
                        return final_card
            if id == 1 and gameinfo['handRemain'][2] == 1:
                return [0] * 15
    ...
    if sum(play_card) == 0:
        if id == 1 and gameinfo['handRemain'][2] == 1:
            ...
            if rocket_len > 0:
                return pos_split['rocket'][0]
        if pos_split['lord_card'] <= 2 or pos_split['other_depth'] == len(pos_split['rocket']) + len(pos_split['bomb']):
            if bomb_len > 0:
                if card_type == 1:
                    return negPlayBomb(gameinfo, pos_split)
                else:
